...
- taskId：TASK-20260921-team-search-watch-watch
- 模型：GPT-5.6-Luna max；不得更换模型。
- State：.ai/state/20260921-team-search-watch.yaml；design revision：tsw-design-r1。
- 输入：.ai/docs/20260921-team-search-watch/{requirement,uispec,impact,architecture-review,design,testcases}.md。
...
- .ai/docs/20260921-team-search-watch/watch-changereport.md。
- .ai/runtime/results/DISPATCH-20260921-TSW-WATCH-A1.json。
...
结果 by=/root/luna_watch_0921，criterion=IMPLEMENTED，revision=tsw-code-r1，附写入口事件覆盖表和 schema 影响。只返回 proposal，不改 State 或声明整体完成。




新增排除：st-team/src/test/java/com/stcloud/team/service/FileWatchReliabilityIntegrationTest.java，由独立可靠性测试任务拥有。

允许必要工具修复：.ai/scripts/compare-schema.ps1，仅修复合法反引号列名解析，禁止弱化比较或跳过read字段。

本次续做结果改为 .ai/runtime/results/DISPATCH-20260921-TSW-WATCH-A2.json，by=/root/luna_watch_0921b。修复当前watch-tests.log编译/测试问题。

### A5 浏览器回归修复
真实团队新建目录触发缺租户捕获异常并回滚。须修复可信租户快照构建并覆盖新建写入口的真实发布回归；不得跳过捕获或放宽跨租户校验。范围沿用本 TASK。
