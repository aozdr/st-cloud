...
- 版本：1.2 / tsw-design-r4
- 作者：GPT-6；本次续做由当前主线程完成集成修正和验收
- 前置文档：requirement.md、uispec.md、impact.md、architecture-review.md
- 状态：集成设计定版；执行状态和限制见同目录测试与复核记录
...
SearchIndexInitializer 增加 `tenantId:long`、`spaceId:long`、`parentId:long`、`fileMd5:keyword`。全文/纯元数据两种写入都填充；updateMeta 同步空间与父 ID 等元数据。ORM 租户插件未回填内存对象时，`ReliableEventPublisher` 在发布索引事件前只使用已认证的 `UserContext.tenantId` 补齐缺失租户，并拒绝与认证租户冲突的节点；后台已带租户快照的事件保留原值。若认证上下文和节点均缺少有效 tenantId，索引事件在写 Outbox 前失败，不使用默认租户或写入不完整快照。旧文档缺少 tenantId/spaceId 时不会匹配新团队过滤，不采用缺字段默认授权。
...
通知列表分页限制 page≥1、size 1～100。列表映射新关注通知会按行实时读取节点并重新核权，必须限制单页最大工作量。

已读接口仍限本人；取消关注不删除历史通知，通知读取仍核权。未读计数仅为本人已有通知条数，不包含其他用户或候选队列数量。

## 7. 前端实现边界

1. SearchPage 分离 personal/team 请求分支，新增 TeamSearchResultPage 类型；切换范围清游标，用序号/取消信号防止旧响应覆盖。追加页游标过期时，即使已有结果也显示“重新搜索”，重置游标并从第一页请求。保留个人搜索原功能。
...
5. NotificationBell 增加安全 target 请求；已读失败保留真实未读状态；错误、不可用不跳转。连续选择多个通知时仅最后一次选择的 target 响应可提示或导航，过时响应丢弃；组件卸载后同样不得导航。保证 keyboard/窄屏状态。
...
| 后端/数据库/事件与权限、前端及测试代码的本轮剩余修改 | 当前 GPT-6 主线程 | st-common/core/team/search/web 及 SQL/H2 |
| 代码/安全/体验检查 | 当前用户要求由主线程自行复核；正式独立门禁仍按 State 的角色分离规则判定 | codereview/security/exp-review |
| 集成构建、测试、数据库对比 | 主线程串行执行 | testreport |
| 知识库与最终验收 | 主线程 | 真实结果与剩余限制 |

当前轮没有并行编码任务；Maven 与数据库 schema 检查由主线程串行执行。用户要求后续工作由当前模型完成，并已中断 r8 Luna 代码复审。此处调整的是执行分工，不改变接口与技术方案。
...
本轮补充（2026-09-23，GPT-6）：索引事件早于同步事件发布，故可信租户补齐必须覆盖索引 Outbox 序列化；缺可信身份时拒绝写入；通知列表的每行核权读取由 size≤100 限制。其他接口与兼容边界不变。
